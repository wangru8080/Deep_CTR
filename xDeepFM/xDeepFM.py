    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('Linear_Layer'):
                self.linear_out = self.feature_value # [None, field_size]
            ...
            with tf.name_scope('DNN_Layer'):
                self.deep_out = tf.reshape(self.embeddings, shape=[-1, self.field_size * self.embedding_size])
            ...
            with tf.name_scope('CIN_Layer'):
                layers = []
                field_nums = []
                final_result = []
                layers.append(self.embeddings)
                field_nums.append(self.field_size)
                self.x0 = tf.split(self.embeddings, num_or_size_splits=[1] * self.embedding_size, axis=2) # embedding_size * [None, field_size, 1]
                for i, layer_size in enumerate(self.cin_layer):
                    self.xk = tf.split(layers[-1], num_or_size_splits=[1] * self.embedding_size, axis=2)
                    self.xk = tf.matmul(self.x0, self.xk, transpose_b=True)
                    self.xk = tf.reshape(self.xk, shape=[self.embedding_size, -1, field_nums[0] * field_nums[-1]])
                    self.xk = tf.transpose(self.xk, perm=[1, 0, 2]) # [None, embedding_size, out]

                    curr_out = tf.keras.layers.Conv1D(filters=layer_size, kernel_size=1, strides=1)(self.xk) # [None, embedding_size, channel_out]
                    curr_out = tf.transpose(curr_out, perm=[0, 2, 1])  # [None, channel_out, embedding_size]
                    final_result.append(curr_out)
                    layers.append(curr_out)
                    field_nums.append(layer_size)

                self.cin_out = tf.concat(final_result, axis=1) # [None, channel_out*cin_layer_size, embedding_size]
                self.cin_out = tf.reduce_sum(self.cin_out, axis=-1)

            with tf.name_scope('out'):
                self.out = tf.concat([self.linear_out, self.deep_out, self.cin_out], axis=1)
...
if __name__ == '__main__':
    ...
    model = xDeepFM(feature_size=dataParse.feature_size,
